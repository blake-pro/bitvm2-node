...
(*                                                                         *)
(* STATUS: FIXED on dev by #455 (978f4201) - handle_kickoff_sent_verifier  *)
(* now defers the SPV-lag case via push_local_unhandled_messages_with_     *)
(* reason(GoatSpvPending), mirroring the committee handler (the design     *)
(* checked by VerifierKickoffFailOpenFixed.cfg). This module is frozen as  *)
(* the regression record of the ORIGINAL bug, described below as it stood  *)
(* at c15f1714.                                                            *)
(*                                                                         *)
(* ORIGINAL BRANCH STATUS: dev (gc-v2) was the default/shipping branch and *)
(* CONTAINED this bug - verified at handle.rs handle_kickoff_sent_verifier *)
(* (the goat_confirmed_btc_height SPV-lag branch returned Ok(()) with no   *)
(* push_local_unhandled; should_always_challenge was never called;         *)
(* detect_kickoff only scanned OperatorDataPushed). The stale origin/HEAD  *)
...
(* calls should_challenge - i.e. main lacks this code path; it was a       *)
(* gc-v2 regression on the branch actually shipped.                        *)
(*                                                                         *)
