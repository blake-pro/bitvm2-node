        ...
        .run(&strat, |_| {
            run_async(async move {
                ...
                test_support::insert_goat_tx(
                    &mut storage,
                    &GoatTxRecord {
                        instance_id: uuid::Uuid::from_str(
                            instance_id_hex.trim_start_matches("0x"),
                        )
                        .unwrap(),
                        graph_id: Uuid::nil(),
                ...
                let instance =
                    test_support::get_instance(&mut storage, &instance_id).await.unwrap();
                ...
                .await
                .unwrap();
                assert_eq!(record.processing_status, GoatTxProcessingStatus::Skipped.to_string());
            });
            Ok(())
        })
        .unwrap();
